      ...
      shop.with_shopify_session do
        ...
        if customer_id.presence
          ...
          response = HTTP.accept("application/json").headers("X-Shopify-Access-Token" => "#{shop.shopify_token}").put("https://#{shop.shopify_domain}/admin/api/2019-04/customers/#{customer_id}.json",
            :json => { :customer => vet_params.merge(metafields: customer_meta).merge(addresses: [address_params.to_h]).merge(password_confirmation: vet_params[:password]).merge(id: customer_id) }
          )
          puts vet_params.merge(metafields: customer_meta).merge(addresses: [address_params.to_h]).merge(password_confirmation: vet_params[:password]).merge(id: customer_id)
